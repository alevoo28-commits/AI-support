    ...
    *,
    ...
    def _rc_pick_agent_id_ui() -> Optional[str]:
        ...
        if not matching:
            ...
            with st.modal("�Ys� Atención: Instala el Diagnóstico Automático en tu PC"):
                st.markdown(f"""
        ...
        with progress_container:
            with st.status("�Y>�️ Remote Control: ejecutando diagnóstico en el PC cliente", expanded=True) as rcst:
                try:
                    ...
                    if status == "done":
                        rcst.update(label="�o. Diagnóstico remoto completado", state="complete")
                    else:
                        rcst.update(label="�O Diagnóstico remoto falló", state="error")
                    ...
                    return (
                        "DIAGN�"STICO AUTOMÁTICO (PC CLIENTE / POLLER)\n"
                    ...
                    rcst.update(label="�O Remote Control: error", state="error")
                    st.write(f"�O Error Remote Control: {e}")
                    return None
        return None
    ...
    with progress_container:
        with st.status("�YO� Precheck de conectividad", expanded=True) as pre:
            try:
                ...
                pre.update(label="�O Precheck: error", state="error")
                st.write(f"�O Error ejecutando precheck: {e}")
    ...
    with progress_container:
        with st.status("�Y"� Diagnosticando conectividad de red...", expanded=True) as status:
            st.write("�Y"� Iniciando diagnóstico de conectividad de red...")
            if not allow_changes:
                st.info("�"�️ Modo diagnóstico: cambios automáticos DESACTIVADOS (solo análisis).")
            
            try:
                # PASO 1: Detectar interfaz Ethernet activa (Up)
                st.write("�Y"Z Paso 1/4: Detectando adaptador Ethernet...")
                if use_agent:
                    adapters = remote_list_adapters(remote_cfg)
                elif use_winrm:
                    adapters = list_net_adapters_remote(winrm_host)
                ...
                if not ethernet_adapter:
                    st.warning("�s�️ No se encontró adaptador Ethernet, buscando alternativa...")
                ...
                if not ethernet_adapter:
                    st.error("�O No se encontró un adaptador de red activo")
                    st.info(f"Adaptadores disponibles: {', '.join([a.get('Name', 'Sin nombre') for a in adapters[:5]])}")
                    status.update(label="�O Diagnóstico con errores", state="error")
                    
                    return f"""ACCI�"N AUTOMÁTICA: No se encontró adaptador Ethernet.

RESULTADO:
�s�️ No se encontró un adaptador Ethernet activo (Up).
...
4. Mantén tu respuesta breve (3-4 líneas)"""
                ...
                st.success(f"�o. Adaptador encontrado: {ethernet_adapter}")
                
                # PASO 2: Probar conectividad (ya sabemos que falló, pero mostramos para UI)
                st.write("�YO� Paso 2/4: Probando conectividad a Internet (8.8.8.8)...")
                st.warning("�s�️ Sin conectividad a Internet (confirmado)")
                
                # PASO 3: Revisar configuración IP
                st.write("�Y"� Paso 3/4: Obteniendo configuración IP actual...")
                if use_agent:
                ...
                if not allow_changes:
                    ...
                    status.update(label="�s�️ Diagnóstico completado (sin cambios)", state="complete")

                    return f"""DIAGN�"STICO DE RED (SIN CAMBIOS AUTOMÁTICOS):
...
Respuesta breve (4-6 líneas)."""
                ...
                if ip_config["has_ip"]:
                    ...
                    st.info(f"�Y"< IP actual: {current_ip} (sin conectividad)")
                    st.write("�sT️ Paso 4/4: Buscando nueva IP en el mismo segmento...")
                    
                    # Extraer segmento (primeros 3 octetos)
                    ip_parts = current_ip.split(".")
                    if len(ip_parts) == 4:
                        segment = f"{ip_parts[0]}.{ip_parts[1]}.{ip_parts[2]}"
                        st.info(f"�Y"� Segmento detectado: {segment}.x")
                        ...
                        st.info(f"�Y"S Encontradas {len(candidates)} IPs disponibles en el segmento")
                        
                        if candidates:
                            new_ip = candidates[0]
                            st.write(f"�sT️ Asignando nueva IP: {new_ip} (mismo segmento {segment}.x)...")
                            ...
                            if result.ok and result.assigned_ip:
                                st.success(f"�o. IP cambiada: {current_ip} �?' {result.assigned_ip}")
                                st.info(f"�Y"� Adaptador: {ethernet_adapter} | Máscara: 255.255.255.0")
                                status.update(label="�o. Diagnóstico completado", state="complete")
                                
                                return f"""ACCI�"N AUTOMÁTICA COMPLETADA:

DIAGN�"STICO:
- Adaptador: {ethernet_adapter}
- IP anterior: {current_ip} (sin conectividad)
- IP nueva: {result.assigned_ip} �o.
- Segmento mantenido: {segment}.x
- Gateway: 172.17.87.1
- DNS: 8.8.8.8, 8.8.4.4

RESULTADO:
�o. IP configurada exitosamente en el mismo segmento de red.
...
4. Mantén tu respuesta concisa (3-4 líneas)"""
                                ...
                                st.error("�O Error al cambiar IP. Permisos de Administrador requeridos.")
                                st.warning(f"�Y'� Detalle: {result.details}")
                                status.update(label="�O Diagnóstico con errores", state="error")
                                
                                return f"""ACCI�"N AUTOMÁTICA: Error al cambiar IP por permisos.

RESULTADO:
�O No se pudo cambiar la IP {current_ip} por permisos insuficientes.
...
3. Mantén tu respuesta concisa (4-5 líneas)"""
                            ...
                            st.error(f"�O No hay IPs disponibles en el segmento {segment}.x")
                            status.update(label="�O Diagnóstico con errores", state="error")
                            
                            return f"""ACCI�"N AUTOMÁTICA: Sin IPs disponibles.

RESULTADO:
�s�️ No hay IPs disponibles en el segmento {segment}.x
...
3. Breve (2-3 líneas)"""
                        ...
                        st.error("�O IP actual inválida")
                        status.update(label="�O Diagnóstico con errores", state="error")
                        return "Error: IP actual inválida"
                else:
                    # No tiene IP asignada �?' Asignar del pool
                    st.info("�Y"< No tiene IP asignada")
                    st.write("�sT️ Paso 4/4: Asignando IP automáticamente del pool...")
                    ...
                    if use_agent or use_winrm:
                        ...
                        status.update(label="�s�️ Diagnóstico completado (requiere IP desde servidor)", state="complete")
                        return """DIAGN�"STICO DE RED (MODO REMOTO):
...
Breve (4-6 líneas)."""
                    ...
                    if result.ok and result.assigned_ip:
                        st.success(f"�o. IP asignada: {result.assigned_ip}")
                        st.info(f"�Y"� Adaptador: {ethernet_adapter} | Máscara: 255.255.255.0")
                        status.update(label="�o. Diagnóstico completado", state="complete")
...
RESULTADO:
- �o. IP configurada exitosamente: {result.assigned_ip}
- Adaptador: {ethernet_adapter}
...
4. Mantén tu respuesta concisa (3-4 líneas)"""
                        ...
                        st.error("�O Error al asignar IP. Permisos de Administrador requeridos.")
                        status.update(label="�O Diagnóstico con errores", state="error")
                        
                        return f"""ACCI�"N AUTOMÁTICA: Error al asignar IP por permisos.

RESULTADO:
�O No se pudo asignar IP por permisos insuficientes.
...
3. Mantén tu respuesta concisa (4-5 líneas)"""
                ...
                st.error(f"�O Error agente remoto: {e}")
                status.update(label="�O Diagnóstico con errores", state="error")
                return f"""DIAGN�"STICO DE RED: Error con agente remoto.

RESULTADO:
�O No pude contactar/usar el agente remoto del PC cliente: {e}
...
3. Mantén tu respuesta breve (3-5 líneas)."""
                ...
                st.error(f"�O Error inesperado: {e}")
                status.update(label="�O Diagnóstico con errores", state="error")
                
                return f"""ACCI�"N AUTOMÁTICA: Error al diagnosticar.
